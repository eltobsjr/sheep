...
import '../webview_page_scraper.dart';

// HTML scraper + hidden-WebView reader for https://mangafire.to (EN)
//
...
//   WebViewPageScraper loads the reader URL in an offstage WebView, injects an
//   XHR interceptor that captures the /ajax/read/ response, and returns image URLs.
//
...
class MangaFireSource extends HttpMangaSource {
  ...
  // MangaFire's reader endpoint requires a VRF token generated by obfuscated JS.
  // We delegate to WebViewPageScraper which loads the reader URL in an offstage
  // WebView, intercepts the /ajax/read/ XHR response, and returns the image URLs.
  @override
  Future<List<String>> getPages(String chapterUrl,
      {bool dataSaver = false}) async {
    final readerUrl = chapterUrl.startsWith('http')
        ? chapterUrl
        : '$baseUrl/$chapterUrl';
    return WebViewPageScraper.instance.scrape(readerUrl, id);
  }
}
